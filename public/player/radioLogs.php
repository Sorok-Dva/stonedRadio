...
if($artist=="Advert:Targetspot") { ?>
    <center><strong>Publicité</strong></center>
<?php } else {
    ?>
<div class="directRadio">
    <center>
        <small>
            <strong>
                <?php
    ...
    <?php if($artist != "Jingle Stoned-Radio "){ ?>
        <div style="margin-top: 7px;text-align:center;">
            <button onclick="Vote('pour', '<?= addslashes($artist) ?>', '<?= addslashes($title) ?>', '<?= addslashes($cover) ?>')" class="btn btn-success"><span class="glyphicon glyphicon-thumbs-up" aria-hidden="true"></span></button> &nbsp; <button onclick="Vote('contre', '<?= addslashes($artist) ?>', '<?= addslashes($title) ?>', '<?= addslashes($cover) ?>')" class="btn btn-danger"><span class="glyphicon glyphicon-thumbs-down" aria-hidden="true"></span></button>
        </div>
  <?php  } ?>
...
if (!empty($_POST) AND ($_POST['action'] == "reloadCover")) {
		$xml = @simplexml_load_file($cache);
		if(empty($xml->track->cover))
		{
			$cover = "http://radionomy.letoptop.fr/images/none.jpg";
